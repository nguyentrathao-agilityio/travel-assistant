...
const FlightInputShape = z.object({
  ...
  adults: z.number().optional().describe('Number of adult passengers (default 1, min 1)'),
  return_date: z
    .string()
    .refine(isValidIsoDate, 'Must be a valid YYYY-MM-DD date')
    .optional()
  ...
  max_stops: z.number().optional().describe('Maximum number of stops (0 = nonstop only)'),
  sort: FlightSortSchema.optional().describe('Sort order for results'),
});

/** Business-rule checks applied on top of {@link FlightInputShape}. */
const refineFlightInput = (data: z.infer<typeof FlightInputShape>, ctx: z.RefinementCtx) => {
  ...
  if (data.max_stops !== undefined && data.max_stops < 0) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: 'max_stops must not be negative',
      path: ['max_stops'],
    });
  }
};
...
export const FlightToolSchema = z.preprocess(stripNulls, FlightInputShape);

/** Full schema, including business rules — re-validated inside the tool handler. */
export const FlightInputSchema = z.preprocess(
  stripNulls,
  FlightInputShape.superRefine(refineFlightInput)
);
